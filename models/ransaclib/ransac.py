...
# call属性能够让当前类像函数一样调用call属性对应的函数
    def __call__(self, matches, logits, K1, K2, gt_model):
        ...
        while iterations < max_iters:
            ...
            if self.sampler_id != 2 and self.sampler_id != 3:
                # 进行最少估计点采样。
                minimal_sample_indices = self.sampler.sample() # 获得采样点的id索引
                minimal_samples = matches[minimal_sample_indices] # 根据采样点缩影获得对应的匹配特征点对
...
# 这里的gt_model是gtpose真是位姿，但是3DRANSAC好像没有使用这个参数
    # 这里的logits就是CLNet网络获得的匹配点属于内点的概率值
    def __call__(self, matches, logits, gt_model, valid=False):
        # print("############## matches shape ", matches.shape," logits shape ", logits.shape)
        # 这里的RANSAC是对每个npz文件中的点云进行独立的求解，所以matches的shape 是 [points_num, 6] 其中6表示两个匹配点各自的三维坐标值
        if valid:
        ...
        # print("############ sampler_id ", self.sampler_id) # 训练的时候这个samplerid好像是等于2的
        while iterations < self.max_iterations:
            ...
            if self.sampler_id != 2 and self.sampler_id != 3:
                ...
                # 训练点云配准的时候用的是这部分
                samples, soft_weights = self.sampler.sample(logits) # sample是论文公式5d中的Y，softweights是公式5c中的y~
                # print("############### samples shape ", samples.shape, "####### softweights shape ", soft_weights.shape) # 这两个shape都是 [ransacbatchsize, pointnum]
                points = matches.repeat([self.ransac_batch_size, 1, 1]) * samples.unsqueeze(-1)
                # print("################### after sampel points ", points.shape) # points 的 shape是 [ransacbatchsize, pointnum, 6]
                minimal_samples = points[samples != 0].view(self.ransac_batch_size, -1, matches.shape[-1]) # 利用sample中元素值不等于0的点作为采样点，其实就是选择top-k排名最靠前的属于内点概率最大的匹配点作为采样点。
            # when there is no minimal sample comes, skip如果没有获得到采样点就跳过当前迭代
            if minimal_samples.shape[1] == 0:
                continue

        # 它这里minimalsample好像是去了ransacbatchsize组数，每组数都有3个特征匹配点
            # print("########### 前两个minimalsamplers的值 ", minimal_samples[0], " dddddd ", minimal_samples[1])
            # print("#$##################### minimal_samplers shape ", minimal_samples.shape) # 这个变量的shape是 [ransacbatchsize, solver求解器的num_sampler属性值, 6]
            # Estimate models' parameters, can propagate gradient
            # self.estimator是SVD求解器，用来根据采样点求解E或F矩阵， 然后再用SVD分解出R、t
            # estimate_model是通过送入匹配点对来求解，同时它能够适用batchsize的形式，所以下面的输入参数为 [ransacbatchsize, point_num, 6] 6表示一个匹配点对的两个三维坐标
            estimated_models, R, t, _ = self.estimator.estimate_model(minimal_samples)
            # print("################## estimated_models ", estimated_models[0]) # 这里的estimatemodel实际就是T矩阵，即4x4完整的位姿变换矩阵T
            # print("####################### estimated_models shape ", estimated_models.shape)
            # print("####################### R, t shape ", R.shape, " ", t.shape)
            ...
            if self.train:
                ...
                # print("############### 每次iteration后获得的模型数量", models[iterations].shape) # [ransacbatchsize, 4,4]就是每次迭代都会获得ransacbatchsize个模型估计结果

                # 下面的代码最后参数estimated_models[:, :3, :]中的:3表示的是把T位姿变换矩阵只取前三行，因为第四行是0 0 0 1没啥用。
                # 为ransacbatchsize个位姿矩阵估计结果分别计算它们对当前matches的内点率和三维距离误差大小，可以理解成计算这些估计出的位姿矩阵对当前matches的适应度
...
# 从这里可以看出，为啥求解R、t和计算误差会出现ransacbatchsize，原因是为了每次迭代多跑出来几个结果，可以在一定程度上利用矩阵乘法的特性，每次迭代多预测几个，提高速度
            # 但是实际上总迭代次数还是那么多，即 iteration * ransacbatchsize = total_iteration
            iterations += self.ransac_batch_size
        ...
        # print("############ best_model ", best_model) # 字典类对象，字典keys是0、64等。然后value是64个估计出来的位姿矩阵
        # print("############ residuals ", residuals) # 和上面的对象一样
