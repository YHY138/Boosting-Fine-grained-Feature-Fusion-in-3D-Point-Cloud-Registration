...
def knn(x, k):
    ...
    # print("############### pairwise_distance ", type(pairwise_distance), pairwise_distance.shape,"k is ", k)
    # print("############## pairwise_distance place ", pairwise_distance.device)
    # 这里有个pytorch自身的问题，好像1.12~1.13的版（1.11和2.0没问题）当tensor的数据量大于600000的时候，调用topk会报错：
    # RuntimeError: CUDA error: an illegal memory access was encountered。就是CUDA访问GPU显存出现泄露或者访问了错误显存位置。
    # 这个问题应该和pytorch版本有关。
...
class RANSACLayer(nn.Module):
    def __init__(self, opt, **kwargs):
        ...
        # 最后创建并保留的是可微分RANSAC类，上面创建的采样器和求解器都作为下面这个类的构建参数。
...
class DS_Block(nn.Module):
    def __init__(self, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.in_channel = 7 # in_channel往往对应输入数据的第二个维度
        ...
        self.conv = nn.Sequential(
            # 因为待处理的数据维度刚好就是4个维度，能用Conv2d处理。然后这里卷积核用的（1，1），因此此处只是起到一个提升数据维度的作用。
    ...
    def forward(self, x):
        # print("######## CLNet input", x.shape)# 原始维度
        B, _, N, _ = x.size()
        out = self.conv(x)
        # print("############# out shape ", out.shape) # 将inchannel换成了outchannel，完成升高维度
...
class RANSACLayer3D(nn.Module):
    def __init__(self, opt, **kwargs):  # weights,
        ...
        if self.opt.sampler == 0:
            ...
            sampler = GumbelSoftmaxSampler( # 这里需要用到ransacbatchsize而不是数据集的batchsize
                opt.ransac_batch_size,
                solver.sample_size,
                device=opt.device,
                data_type=data_type
        ...
        scoring = MSACScore(self.opt.device) # 这时用来求RANSAC3D计算出的模型参数能适应于多少匹配特征的损失

        # maximal iteration number, fixed when training, adaptive updating while testing
        max_iters = 200
        # print("################ ransac batchsize ", opt.ransac_batch_size) # 这个参数默认是64
...
# 不知道为啥，3D RANSAC可以不提供groundtruth
    def forward(self, points, weights, ground_truth=None):
        # print("############# points shape ", points.shape, "######## weights shape ", weights.shape)
        start_time = time.time()
        models, residuals, avg_residuals, model_score, iterations = self.estimator(points, weights, ground_truth)
        # print("######### models shape", type(models), " models keys ", models.keys()) # models是一个字典类型
        # print("########### iterations ", type(iterations)," !!!! ", iterations) # iteration是一个int整数类型,1024
        ransac_time = time.time() - start_time
...
class CLNet(nn.Module):
    ...
    def forward(self, points, prob_type=0):
        # print("############# CLNet points shape ", points.shape) #[batchsize, 7（两个特征点的三维坐标+匹配置信度）, 匹配点对的个数, 1]

        B, _, N, _ = points.shape
        #import pdb; pdb.set_trace()

        w1 = self.ds_0(points)
        # print("############### w1 shape ", w1.shape) # [32, 4000] # w1就是CLNet网络输出的每个场景数据下网络对各个匹配点对是否是内点的判断
