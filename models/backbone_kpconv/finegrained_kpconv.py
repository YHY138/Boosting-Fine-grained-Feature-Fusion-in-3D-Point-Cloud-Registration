...
from .finegrained_kpconv_blocks import *
import logging
...
class KPFEncoder(torch.nn.Module):
    def __init__(self, config, d_bottle, increase_channel_when_downsample=True):
        ...
        number_of_layers = len(config.architecture)
        sti, edi = 2, number_of_layers - 2
        if sti > edi:
            sti = edi = number_of_layers / 2
    ...
    def forward(self, x, batch):
        ...
        for block_i, block_op in enumerate(self.encoder_blocks):
            # print('Block Number {}'.format(block_i))
