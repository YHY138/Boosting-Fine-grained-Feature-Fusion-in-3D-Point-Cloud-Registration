...
def block_decider(block_name,
    ...
    if block_name == 'unary':
        ...
        if flag:
            return ResnetBottleneckBlock(block_name, in_dim, out_dim, radius, layer_ind, config, True)
        else:
...
class ResnetBottleneckBlock(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config, flag=False):
        """
        ...
        # KPConv block
        self.KPConv = KPConv(config.num_kernel_points,
                             config.in_points_dim,  # 三维点的维度，一般是三维
                             out_dim // 4,
                             out_dim // 4,
                             current_extent,
                             radius,
        ...
        self.res2net = my_res2Net(my_Bottle2neck, out_dim // 4, out_dim, baseWidth=14, scale=8)
    ...
    def forward(self, features, batch):
        ...
        # Convolution
        x = self.KPConv(q_pts, s_pts, neighb_inds, x)
        x = self.batch_norm_conv(x, stack_lengths_post)
        x = self.res2net(x)
        x = self.leaky_relu(x)
