'''
This is designed to construct a pkl dataset compatible with Regtr training using the SemanticMCD dataset, 
...
pth_files = sorted(glob.glob(exported_dir + '/inL_labelled_Sample_5kpts/*.pth'))
pose_files = sorted(glob.glob(exported_dir + '/inL_labelled_Sample_5kpts/*_pose.txt'))
label_files = sorted(glob.glob(exported_dir + '/inL_labelled_Sample_5kpts/*_label.txt'))
...
kitti_pkl = {}
kitti_pkl['src'] = np.array(src[:train_num])
kitti_pkl['tgt'] = np.array(tgt[:train_num])
kitti_pkl['overlap'] = np.array(overlap[:train_num])
kitti_pkl['rot'] = np.array(rot[:train_num])
kitti_pkl['trans'] = np.array(trans[:train_num])
kitti_pkl['src_labels'] = np.array(src_labels[:train_num], dtype=object)
kitti_pkl['tgt_labels'] = np.array(tgt_labels[:train_num], dtype=object)

val_kittipkl = {}
val_kittipkl['src'] = np.array(src[train_num:])
val_kittipkl['tgt'] = np.array(tgt[train_num:])
val_kittipkl['overlap'] = np.array(overlap[train_num:])
val_kittipkl['rot'] = np.array(rot[train_num:])
val_kittipkl['trans'] = np.array(trans[train_num:])
val_kittipkl['src_labels'] = np.array(src_labels[train_num:], dtype=object)
val_kittipkl['tgt_labels'] = np.array(tgt_labels[train_num:], dtype=object)
